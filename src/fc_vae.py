...
class Encoder(layers.Layer):
    ...
    def __init__(self, input_channels, latent_channels, name="encoder", **kwargs):
        ...
        self.mean = layers.Dense(latent_channels*7*7)
        self.log_var = layers.Dense(latent_channels*7*7)
...
class Decoder(layers.Layer):
    ...
    def __init__(self, output_channels, latent_channels, name="decoder", **kwargs):
        ...
        self.cout = layers.Conv2D(output_channels, kernel_size=1,
                activation='sigmoid')
        self.reshape = layers.Reshape(target_shape=(7, 7, latent_channels))
        self.fc1 = layers.Dense(256, activation='relu')
        self.fc2 = layers.Dense(7*7*latent_channels, activation='relu')
...
# Iterate over epochs.
if not os.path.isfile('fc_vae.index'):
    ...
    vae.save_weights('fc_vae')
else:
    vae.load_weights('fc_vae')

plt.style.use('dark_background')
fig, ax = plt.subplots(ncols=4, nrows=4, figsize=(4,4))
for i in range(4):
    for j in range(4):
        rand_code = tf.random.normal(shape=(1, 490))
...
plt.savefig('fc_vae.png', bbox_inches='tight')
plt.show()
